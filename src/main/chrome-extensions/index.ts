...
const EXTENSION_FOLDER = (function extensionFolder() {
  ...
  if (platform === "darwin") {
    folderPath = path.resolve(app.getPath("home"), "Library/Application Support/Google/Chrome/Default/Extensions");
  }

  if (platform === "win32") {
    folderPath = path.resolve(app.getPath("home"), "AppData\\Local\\Google\\Chrome\\User Data\\Default\\Extensions");
  }

  if (platform === "linux") {
    const availablePath = [".config/google-chrome/Default/Extensions/", ".config/google-chrome-beta/Default/Extensions/", ".config/google-chrome-canary/Default/Extensions/", ".config/chromium/Default/Extensions/"].map(p => path.resolve(app.getPath("home"), p));
...
function addDevToolsExtension(id: string) {
  ...
  const versionName = readdirSync(extensionPath).find(v => existsSync(path.resolve(extensionPath, v)) && /\d+\.\d+\.\d/.test(v));
